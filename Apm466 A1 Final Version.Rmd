...
#### 2.
The selected 11 bonds are: CAN 0.50 Mar 22, CAN 0.25 Aug 22, CAN 1.75 Mar 23, CAN 0.25 Aug 23, CAN 2.25 Mar 24, CAN 1.50 Sep 24, CAN 1.25 Mar 25, CAN 0.50 Sep 25, CAN 0.25 Mar 26, CAN 1.00 Sep 26, CAN 1.25 Mar 27. The maturity dates of 11 selected bonds are from March 1, 2022 to March 1 to 2027, a five period from now. Since they have semi-annual coupon payments, I choose those bonds with maturity dates close to 0.5, 1, 1.5, …, 4.5, 5 years from now, which allows me to calculate the yield curves using the bootstrapping technique. Also, most of those bonds have a duration of 5 years and coupon rates ranging from 0.25% to 2.25%.
...
plot(forwards_matrix[,1],type = "l",ylab = "Spot Rate", xlab="Time to Maturity", col="red",main = "Figure 3: Forward Curves Corresponding to Each Day",cex.main=0.8,cex.lab=0.7,cex.axis=0.7,ylim = c(0.012,0.02),font.main = 1,)
...
eigenvector_ytm=eigen_ytm$vectors
eigenvalues_forward=eigen_forward$values
eigenvector_forward=eigen_forward$vectors
kable(eigenvalues_ytm, caption = "Eigenvalues of Covariance Matrix (YTM)") %>%
  kable_styling(latex_options = "HOLD_position",font_size = 7)

kable(eigenvector_ytm, caption = "Eigenvector of Covariance Matrix (YTM)") %>%
  kable_styling(latex_options = "HOLD_position",font_size = 7)

kable(eigenvalues_forward, caption = "Eigenvalues of Covariance Matrix (Forward)") %>%
  kable_styling(latex_options = "HOLD_position",font_size = 7)

kable(eigenvector_forward, caption = "Eigenvector of Covariance Matrix (Forward)") %>%
  kable_styling(latex_options = "HOLD_position",font_size = 7)

```

The first eigenvalue of ytm covariance matrix is 2.973265e-03, corresponding to the eigenvector (-0.60628562,-0.010818930,0.615602835,0.44030231,-0.24386004), indicating the in the direction of eigenvectors, the variance of this time series is 2.973265e-03.

The first eigenvalue of forward covariance matrix is 1.876378e-03, corresponding to the eigenvector (-0.5295236,0.75182422,0.04325555,-0.3905048), indicating the in the direction of eigenvectors, the variance of this time series is 1.876378e-03.
